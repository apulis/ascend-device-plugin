...
func (s *pluginAPI) Allocate(ctx context.Context, requests *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse,
	...
	for _, rqt := range requests.ContainerRequests {
        ...
        ips, err := getNPUIPString(ascendVisibleDevices)        
        if err == nil {

            addIpEnv(ips, resp)
            logger.Info(fmt.Sprintf("getNPUIPString succ: %s", ips))
        } else {

            logger.Info(fmt.Sprintf("getNPUIPString err: %v", err))
            return nil, err
        }
...
func addIpEnv(npuIps string, resp *pluginapi.ContainerAllocateResponse) {
	// add env
    if (*resp).Envs == nil {
...
func addEnv(ascendVisibleDevices string, resp *pluginapi.ContainerAllocateResponse) string {
	...
	(*resp).Envs[VISIBLE_DEVICE_IDS] = ascendVisibleDevices
...
func getNPUIPString(deviceIds string) (string, error) {

    var ip string
    ips := make([]string, 0)

    idSplit := strings.Split(deviceIds, ",")
    sort.Sort(sort.StringSlice(idSplit))
    ...
    for _, deviceID := range idSplit {
        ...
        logicID64, err := strconv.ParseInt(deviceID, 10, 32)
        if err != nil {
            logger.Error("Get device ip failed:->", zap.Int("deviceId", int(logicID64)), zap.Error(err))
            return "", err
        }
        ...
        ip = fmt.Sprintf("%d:%s", logicID, deviceIP) 
        ips = append(ips, ip)
    }
