...
docker_image_prefix="harbor.sigsus.cn:8443/sz_gongdianju"

osname=$(grep -i ^id= /etc/os-release| cut -d"=" -f2 | sed 's/"//g');
ostype=$(arch)
if [ "${ostype}" = "aarch64" ]; then
  ostype="arm64"
  docker_images_name=${docker_images_name}-arm64
else
  ostype="x86"
fi
...
function push_docker_images() 
{
    ...
    docker tag ${docker_images_name} ${docker_image_prefix}/${docker_images_name}
    docker push ${docker_image_prefix}/${docker_images_name}
